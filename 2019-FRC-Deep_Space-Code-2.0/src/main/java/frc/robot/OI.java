...
public class OI {
  ...
  public OI(){
	...
	A.whileHeld(new ForwardBothBelts());
	B.whileHeld(new ReverseBothBelts());
	Y.whenPressed(new Hatch());
	X.whileHeld(new EncoderDrive());
	dUp.whenPressed(new CV_Pivot());
	dDown.whenPressed(new FD_Pivot());
	RB.whenPressed(new HG_Extender());
	dRight.whenPressed(new Drive2Pannel());
	dLeft.whenPressed(new HG_Claw());
	LB.whileHeld(new LimeDrive());
  }	
}
